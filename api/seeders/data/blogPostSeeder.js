...
/**
 * Create or find blog categories
 */
...
const getAdminUser = async () => {
  ...
  if (!adminUser) {
    throw new Error('No admin user found. Please seed users first or create an admin user.');
  }
...
const seedBlogPosts = async () => {
  try {
    ...
    console.log('   📊 Blog posts by status:');
    Object.entries(statusCounts).forEach(([status, count]) => {
      console.log(`      ${status}: ${count}`);
    });

    const featuredCount = createdPosts.filter(post => post.is_featured).length;
...
const cleanBlogPosts = async () => {
  try {
    ...
    console.error('   ❌ Error cleaning blog posts:', error.message);
    throw error;
  }
};
